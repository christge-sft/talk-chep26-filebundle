...
an opt-in feature in CernVM-FS 2.14.
}
...
CernVM-FS~\cite{ref-cvmfs} is a global, read-only file system for
distributing scientific software. Rather than downloading and unpacking
a complete software stack in advance, clients access a global namespace
under \texttt{/cvmfs}. Files are fetched when they are opened;
downloading, decompression, and caching are transparent to the user.

CernVM-FS is a core component of the Worldwide LHC Computing Grid and
is also used by communities beyond high-energy physics, including
Euclid, LIGO, LSST, SKA, and EESSI. The infrastructure has about
fifteen Stratum~1 mirror servers in Europe, the Americas, and Asia.
The \texttt{/cvmfs} namespace contains about $3.3\times10^{10}$ files
and 2~PB of data; it also serves more than 8000 container images.
...
interactive CernVM-FS use can be roughly three to five times slower
...
CernVM-FS repository served from \texttt{localhost}. The client ran on
...
CernVM-FS with a cold cache and no filebundles, and on CernVM-FS with
filebundles enabled. In this benchmark, filebundles bring CernVM-FS
...
FUSE passthrough in CernVM-FS~2.14 on recent kernels can hand the
...
needs to be benchmarked. Planned work for CernVM-FS~2.15 includes tools
...
opt-in CernVM-FS~2.14 feature, enabled per mount with
...
\bibitem{ref-docs}
CernVM-FS documentation. \url{https://cvmfs.readthedocs.io/}
